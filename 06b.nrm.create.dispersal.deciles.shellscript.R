...
# EMG this will change the samp.spp for bird so cannot use them together
...
#	for (sp in species.names) { # cycle through each of the species
	for (sp in species.names[samp.spp[[which(taxa==taxon)]]]) {
	
		# create the species specific working directory
		sp.wd = paste(taxon.dir, "/models/", sp, "/1km", sep="")

		# create the shell file
		shell.file.name = paste(sp.wd, "/06b.nrm.create.dispersal.deciles.", sp, ".sh", sep="")
		...
		shell.file = file(shell.file.name, "w")
			...
			cat("R CMD BATCH --no-save --no-restore '--args wd=\"", wd, "\" taxon=\"", taxon, "\" sp=\"", sp, "\"' ", script.dir, "/06b.nrm.create.dispersal.deciles.R ", sp.wd, "/06b.nrm.create.dispersal.deciles.", sp, ".Rout \n", sep="", file=shell.file)
		close(shell.file)
