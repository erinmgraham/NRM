# shellscripts to create deciles suitability scripts
...
for (taxon in taxa) {
	...
	for (sp in species.names) { # cycle through each of the species
		...
		shell.file.name = paste(pbs.dir, "/04b.nrm.suitability.deciles.", sp, ".sh", sep="")
		...
		shell.file = file(shell.file.name, "w")
			...
			cat("R CMD BATCH --no-save --no-restore '--args wd=\"", wd, "\" taxon=\"", taxon, "\" sp=\"", sp, "\"' ", script.dir, "/04b.nrm.suitability.deciles.R ", pbs.dir, "/04b.nrm.suitability.deciles.", sp, ".Rout \n", sep="", file=shell.file)
		close(shell.file)
