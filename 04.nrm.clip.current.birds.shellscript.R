...
for (sp in species.names[-c(28,47,50,88,167,225,396,492,557,558)]) { # cycle through each of the species
	...
	shell.file.name = paste(sp.wd, "/04.nrm.clip.current.birds.", sp, ".sh", sep="")
	...
	shell.file = file(shell.file.name, "w")
		...
		cat("R CMD BATCH --no-save --no-restore '--args wd=\"", wd, "\" taxon=\"", taxon, "\" sp=\"", sp, "\"' ", script.dir, "/04.nrm.clip.current.birds.R ", sp.wd, "/04.nrm.clip.current.birds.", sp, ".Rout \n", sep="", file=shell.file)
	close(shell.file)
